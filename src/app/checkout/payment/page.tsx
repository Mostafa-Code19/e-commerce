...
const Payment = () => {
    ...
    useEffect(() => {
        ...
        if (coupon) {
            if (coupon.type == 'PERCENTAGE') { couponValue = ((coupon.value * (price - discount)) / 100) }
            else if (coupon.type == 'PRICE') { couponValue = coupon.value }
            else return
        }
                                        ...
                                        <div className='flex justify-between'>
                                            <div className='flex space-x-1'>
                                                ...
                                                <span className='text-red-500 font-semibold'>
                                                    {(discount + couponValue).toLocaleString()} (%{Math.round(((discount + couponValue) * 100) / price)})
                                                </span>
                                            </div>
                                            <span className='text-red-500 font-semibold'>
                                                سود شما از خرید
...

                                    {
                                        couponValue ?
                                            <div className='text-center mt-5'>
                                                <span className='text-green-700'>کد تخفیف با موفقیت اعمال شد: {couponValue.toLocaleString()} تومان</span>
                                            </div>
                                            : ''
                                    }
